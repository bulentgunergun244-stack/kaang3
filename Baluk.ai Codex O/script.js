...
const DEFAULT_BLM_NOTEBOOK = `baslik "Baluk Engine"
yaz "BLM not defterine hos geldin."
kart "Ipucu" "Study panelinden hazir komutlar ekleyebilirsin."`;
...
function renderEngineLog() {
  ...
  engineLog.innerHTML = "";
  engineIntro.classList.toggle("hidden", entries.length > 0);
  engineLog.classList.toggle("hidden", entries.length === 0);
  entries.forEach((entry) => {
    const card = document.createElement("article");
    card.className = `engine-log-entry ${entry.role || "assistant"}`;
...
function renderBlmPreview(source = "") {
  ...
  const pieces = [];
  const pickQuoted = (line) => [...line.matchAll(/"([^"]*)"/g)].map((match) => match[1]);
  lines.forEach((rawLine) => {
    const line = rawLine.trim();
    if (!line) return;
    const lower = line.toLocaleLowerCase("tr-TR");
    const quoted = pickQuoted(line);
    if (lower.startsWith("baslik")) {
      pieces.push(`<h2>${escapeHtml(quoted[0] || line.replace(/^baslik\s*/i, ""))}</h2>`);
      return;
    }
    if (lower.startsWith("yaz")) {
      pieces.push(`<p>${escapeHtml(quoted[0] || line.replace(/^yaz\s*/i, ""))}</p>`);
      return;
    }
    ...
    if (lower.startsWith("giris")) {
      pieces.push(`<input type="text" placeholder="${escapeHtml(quoted[0] || "Bir sey yaz")}" />`);
      return;
    }
    if (lower.startsWith("buton")) {
      pieces.push(`<button class="blm-inline-btn" type="button">${escapeHtml(quoted[0] || "Tikla")}</button>`);
      return;
    }
  ...
  return `<div class="blm-stage">${pieces.join("") || `<div class="blm-card">BLM cikti yok.</div>`}</div>`;
}
  ...
  if (enginePanelMode === "study") {
    ...
    engineDockBody.innerHTML = `
      <div class="engine-study-card">
        ...
        <p>BLM sade komutlarla ilerler. Study tarafindan ornekleri not defterine atip T panelinde sonucu izleyebilirsin.</p>
        <div class="engine-study-grid">
          <button type="button" data-study-insert='baslik "Merhaba"\nyaz "Baluk Engine ile ilk satirim"'>Baslik ve yazi</button>
          <button type="button" data-study-insert='giris "Adini yaz"\nbuton "Hesapla"'>Giris ve buton</button>
          <button type="button" data-study-insert='topla 1 1\nyaz "Matematik ogreniyorum"'>Matematik</button>
        ...
        <div class="engine-study-rule">BLM temel komutlari: <b>baslik</b>, <b>yaz</b>, <b>kart</b>, <b>giris</b>, <b>buton</b>, <b>topla</b>, <b>hesapmakinesi</b>, <b>yilanoyunu</b>.</div>
      </div>
    `;
...
function appendEngineEntry(role, text, meta = "") {
  const active = ensureActiveEngineSession();
  active.entries.push({ role, text, meta, at: Date.now() });
...
function buildBlmEngineArtifact(prompt = "", advanced = false) {
  ...
  if (intent.wantsSnake) {
    code = `baslik "Snake Arena"\nyaz "BLM sahnesi hazirlaniyor."\nyilanoyunu`;
    ...
    code = `baslik "Baluk Hesap Makinesi"\nyaz "BLM ile hesap makinesi arayuzu"\nhesapmakinesi`;
    summary = "BLM icin hesap makinesi sahnesi olusturdum.";
  } else if (intent.wantsButton) {
    code = `baslik "Buton Ornegi"\nyaz "Asagida bir buton olusturuldu."\nbuton "Tikla"`;
    ...
    code = advanced ? `baslik "Baluk Engine BLM"\nkart "Mod" "Thinking secili oldugu icin daha dolu bir taslak hazirlandi."\ngiris "Komut yaz"\nbuton "Uret"\nyaz "T paneli ciktiyi, N paneli ise kaynak kodu gosterir."` : `baslik "Baluk Engine BLM"\nyaz "BLM ile hizli baslangic"`;
...
function getEngineStatuses(language = "html", model = "baluk-2.2") {
  ...
  return model === "baluk-2.2-thinking"
    ? ["Kod yaziliyor...", `${languageLabel} baglaniyor...`, "Duzen ve stil genisletiliyor...", "Son kontroller yapiliyor..."]
    : ["Kod yaziliyor...", `${languageLabel} ortami kuruluyor...`, "Temel parcilar diziliyor..."];
}
async function runEngineStatuses(statuses = []) {
...
async function submitEnginePrompt() {
  ...
  enginePrompt.value = "";
  await runEngineStatuses(getEngineStatuses(active.language, active.model));
